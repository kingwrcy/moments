<template>
  <div class="header relative mb-8 ">
      ...
      <div class="userinfo flex flex-col">
        <div class="flex flex-row items-center gap-4 justify-end">
          ...
          <img :src="getImgUrl(res?.data?.avatarUrl!)" class="avatar w-[70px] h-[70px] rounded-xl"
            @click="navigateTo('/')" :class="{ 'cursor-pointer': route.path.startsWith('/detail') }" />
        </div>
    ...
    <div class="absolute right-2 top-2 rounded p-1 flex flex-row gap-2">
      <ClientOnly>
        <Sun color="#FDE047" :size="20" class="cursor-pointer" v-if="colorMode === 'dark'"
          @click="colorMode = 'light'" />
        <MoonStar color="#FDE047" :size="20" class="cursor-pointer" v-else @click="colorMode = 'dark'" />
      </ClientOnly>
    </div>
